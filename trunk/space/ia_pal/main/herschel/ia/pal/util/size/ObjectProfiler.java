 ...
 * as described in <A HREF="http://www.javaworld.com/javaworld/javaqa/2003-12/02-qa-1226-sizeof.html">the article</A>. 
 * See individual methods for details.<P>
 * 
 * This class works using introspection, and basically enumerating all 
 * primitive types, references, etc, and summing them all up to get the size.
 * 
 * A lot of functionality for analyzing memory usage is provided. For the HCSS, 
 * the most important thing to know is to use the profile(...) method of this 
 * class to obtain the size of objects.<P>
 * 
 * This implementation is J2SE 1.4+ only. You would need to code your own
 * identity hash map to port this to earlier Java versions.<P>
 * 
 ...
 * from your main application code. The minimum set of permissions necessary
...
abstract class ObjectProfiler
{
    ...
    // The current values are OK for most 32-bit JVMs, which is what we want. We want a size
    // that is (close to) correct in most JVMs.
    ...
    /*
    public static final int OBJECT_SHELL_SIZE   = Sizeof.objectShellSize();
    public static final int OBJREF_SIZE         = Sizeof.objectRefSize();
    
    public static final int LONG_FIELD_SIZE     = Sizeof.longSize();
    public static final int INT_FIELD_SIZE      = Sizeof.intSize();
    public static final int SHORT_FIELD_SIZE    = Sizeof.shortSize();
    public static final int CHAR_FIELD_SIZE     = Sizeof.charSize();
    public static final int BYTE_FIELD_SIZE     = Sizeof.byteSize();
    public static final int BOOLEAN_FIELD_SIZE  = Sizeof.booleanSize();
    public static final int DOUBLE_FIELD_SIZE   = Sizeof.doubleSize();
    public static final int FLOAT_FIELD_SIZE    = Sizeof.floatSize();
    */
    ...
    private static ObjectProfileNode createProfileTree (Object obj,
                                                        final IdentityHashMap<Object, ObjectProfileNode> visited,
                                                        final Map<Class<?>, ClassMetadata> metadataMap)
    {
        final ObjectProfileNode root = new ObjectProfileNode (null, obj, null);
        
        final LinkedList<ObjectProfileNode> queue = new LinkedList<ObjectProfileNode> ();
    ...
    {
        final LinkedList<IObjectProfileNode> queue = new LinkedList<IObjectProfileNode> ();
     ...
     */
    private static ClassMetadata getClassMetadata (Class<?> cls,
                                                   final Map<Class<?>, ClassMetadata> metadataMap,
